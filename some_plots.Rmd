...
g2 <- ggplot(MCU_films, aes(x = release_date, y = World_Wide, color = budget_USD, text = actor_name)) +
  geom_point()
...
gBvB <- ggplot(data= MCU_films, aes(x= budget_USD, y= World_Wide, text = title)) +
  geom_point()
...
award_nom_count <- filmography_awards_masterfile %>%
  group_by(actor_name) %>%
  summarize(Number_of_Awards = length(na.omit(winner)))
...
MCU_film_count <- master_MCU %>%
  group_by(actor_name) %>%
  summarize(MCU_film_total = length(title), MCU_Avg_BO = mean(World_Wide, na.rm= TRUE), MCU_Avg_Budget = mean(budget_USD, na.rm= TRUE))

Non_film_count <- master_Non %>%
  group_by(actor_name) %>%
  summarize(Non_film_total = length(title), Non_Avg_BO = mean(World_Wide, na.rm= TRUE), Non_Avg_Budget = mean(budget_USD, na.rm= TRUE))


PC_data <- award_nom_count %>%
  left_join(MCU_film_count) %>%
  left_join(Non_film_count)

head(PC_data)

ggparcoord(PC_data, columns= c(1, 3, 6, 4, 7, 5, 8, 2), alphaLines= .5, scale= "uniminmax")  +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
