...
for(i in 1:length(title_dict)){
    ...
    if(is.null(year)){
      year <- 0 # keep nulls to make list right length
    }
    ...
    rows_add <- data.frame(title_id, title, year, release_date)
...
write_csv(release_df_clean, "Release-Dates.csv")
